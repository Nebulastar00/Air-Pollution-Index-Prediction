...
library(forecast)
library(caTools)
library(rpart) # Change according to needs
...
# Fitting Decison Tree Regression to the Training set
regressor = rpart(formula =  PM2.5 ~ Hour + Temp + WindSpeed + WindDirection 
                 + prevPM2.5,
                  data = training_set,
...
ggplot() +
  ...
  ggtitle('Comparing predicted and actual values of test set') +
  xlab('Date and Time') +
  ylab('PM2.5') 
